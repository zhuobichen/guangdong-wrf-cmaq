...
VARIABLE_CONFIGS = {
    'O3': {
        ...
        'column_name': 'O3',
        'unit': 'μg/m³',
        'file_pattern': 'Emission',  # CMAQ输出文件前缀
        'stats_type': 'Mean',
    ...
    'O3_Days': {
        'display_name': r'O3 Exceedance Days',
        'column_name': 'O3_Days',
        'unit': 'days',
        'file_pattern': 'Emission',
        'stats_type': 'Count',
        ...
        'colorbar_format': '.0f',
        'show_domain_stat': True,
        'is_count_data': True  # 标记为计数数据
    },
    'PM2.5_Days': {
        'display_name': r'PM2.5 Exceedance Days',
...
CMAQ_COMPARISON_CONFIGS = [
    ...
    # 如果有更多对比组合，可以继续添加
    # [
    #     '/path/to/2023_Emission[2030met]_01.csv',  # 2023年1月，2030气象
    #     '/path/to/2023_Emission[2000met]_01.csv',  # 2023年1月，2000气象
    #     2030,        # 文件1对应年份
    #     2000,        # 文件2对应年份
    #     '2030met_2000met_Jan'  # 输出文件后缀
    # ],
]

def extract_month_from_filename(filename):
    """从CMAQ输出文件名中提取月份（格式：YYYY_Emission[YYYYmet]_MM.csv）"""
    # 匹配YYYY_Emission[YYYYmet]_MM.csv格式
    match = re.search(r'_(\d{2})\.csv$', os.path.basename(filename))
    if match:
        month = int(match.group(1))
        if 1 <= month <= 12:
            return month, None  # 不需要年份，因为从文件名中获取
    return None, None  # 如果无法提取，返回None

def generate_plot_title(var_name, year1, year2, month, output_suffix=""):
    """生成CMAQ变量差值图的标题"""
    ...
    # 根据输出后缀确定标题
    #清单变化
    if '2000e2023m_2023e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): (2000e - 2023e)2023m"
    #气象变化
    elif '2023e2000m_2023e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): 2023e(2000m - 2023m)"
    elif '2000e2000m_2023e2000m' in output_suffix:
        title = f"{display_name} ({month_name}): (2000e - 2023e)2000m"
    elif '2000e2000m_2000e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): 2000e(2000m - 2023m)"
    #CMAQ模拟效果变化
    elif '2000e2000m_2023e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): 2000 - 2023"
    elif '2030e2030m_2023e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): 2030 - 2023"
    elif '2060e2060m_2023e2023m' in output_suffix:
        title = f"{display_name} ({month_name}): 2060 - 2023"
    else:
        title = f"{display_name} ({month_name}): {year1} - {year2}"

    return title

def generate_output_filename(var_name, year1, year2, month, output_suffix=""):
    """生成CMAQ变量差值图的输出文件名"""
    ...
    month_name = month_names.get(month, f'M{month:02d}')

    # 变量名映射和显示格式
    var_mapping = {
        'O3': 'O3_Conc',
        'PM2.5': 'PM2.5_Conc',  # 修改：使用PM2.5而不是PM25
        'O3_Days': 'O3_Exceed',
        'PM2.5_Days': 'PM2.5_Exceed'  # 修改：使用PM2.5而不是PM25
    }

    var_display = var_mapping.get(var_name, var_name)

    # 直接使用配置中的输出后缀，不重新生成
    # 例如：output_suffix = "2000e2023m_2023e2023m_Jul"
    filename = f"{var_display}_{output_suffix}.png"

    return filename

def plot_cmaq_difference_map(data1, data2, var_name, file_config, model_file, save_dir, boundary_json_file):
    """
    绘制CMAQ变量差值地图
    """
    ...
    plot_title = generate_plot_title(var_name, year1, year2, month, output_suffix)
    output_filename = generate_output_filename(var_name, year1, year2, month, output_suffix)
...
def process_cmaq_comparison(file_config, var_name, model_file, boundary_file, save_dir):
    ...
    save_file = plot_cmaq_difference_map(
        data1, data2, var_name, file_config, model_file, save_dir, boundary_file
    )
...
def main():
    ...
    VARIABLES_TO_PROCESS = ['O3','O3_Days']
