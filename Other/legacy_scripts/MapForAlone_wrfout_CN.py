...
def plot_multi_year_maps(
    ...
    try:
        ...
        print(f"模型文件读取失败：{str(e)}")
        print("尝试从WRF wrfout文件读取 XLAT/XLONG 作为网格经纬度（投影使用 PlateCarree）...")
        try:
            with nc.Dataset(model_file) as ds:
                if "XLONG" not in ds.variables or "XLAT" not in ds.variables:
                    raise KeyError("WRF文件中缺少 XLONG/XLAT 变量")
                longitudes = ds.variables["XLONG"][0, :, :]
                latitudes = ds.variables["XLAT"][0, :, :]
            proj = None  # show_maps内部会默认使用 PlateCarree
            grid_shape = longitudes.shape
            print(f"成功读取WRF网格经纬度，网格形状：{grid_shape[0]} 行 × {grid_shape[1]} 列")
        except Exception as e2:
            print(f"WRF网格读取失败：{str(e2)}，退出程序")
            return
    ...
    for year in years:
        for month in months:
            ...
            # 1. 拼接当前年月的指标文件路径
            # 兼容两种格式：
            # - 旧格式：{year}_mcipout_{month}.csv
            # - WRF月均2m温度格式：d03_T2_monthly_mean_{year}-{month}.csv（列名：ROW,COL,2mTEMP）
            fusion_output_file = os.path.join(root_metrics_dir, f"{year}_mcipout_{month}.csv")
            if not os.path.exists(fusion_output_file):
                fusion_output_file = os.path.join(root_metrics_dir, f"d03_T2_monthly_mean_{year}-{month}.csv")
            if not os.path.exists(fusion_output_file):
            ...
            for variable in available_vars:
                ...
                if variable_grouped_legend:
                    ...
                    if len(var_data) > 0:
                        ...
                        current_vmax = np.nanpercentile(var_data, 99.5)
                        current_value_range = (current_vmin, current_vmax)
                        print(f"  {year}年{month}月 {variable} 独立图例范围：{np.round(current_vmin, 2)} ~ {np.round(current_vmax, 2)} {current_unit}")
                    else:
                        current_value_range = (0, 1)  # 默认范围
...
if __name__ == "__main__":
    ...
    TARGET_VARIABLES = ['2mTEMP']

    # 4. 图例配置
    UNIFIED_VALUE_RANGE = None  # 图例范围（None则自动计算，或手动指定如(0, 150)）
    UNIFIED_LEGEND = False # 图例模式：True=统一图例，False=分开计算（默认）
    VARIABLE_GROUPED_LEGEND = False  # 单变量无需按组统一
