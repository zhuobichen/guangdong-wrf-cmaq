...
def get_variable_groups():
    ...
    return {
        ...
        'temp_exceed': ['Temp_Days_35C','Heatwave_Days']  # 温度超标天数单独一组
    }
...
def plot_multi_year_maps(
    ...
    if variable_settings is None:
        variable_settings = {
            # 处理输入数据中的所有气象变量
            'variables': ['TA_mean', 'SOL_RAD_mean', 'PBLH_mean', 'TA_max', 'SOL_RAD_max', 'PBLH_max','Heatwave_Days'],
            'settings': {
                'unit': "°C",                  # 默认单位，温度用°C
    ...
    # -------------------------- 图例范围处理（支持统一、分开和分组三种模式） --------------------------
    variable_group_ranges = {}  # 用于存储按变量分组的图例范围

    if variable_grouped_legend:
        # 按变量分组统一图例模式：为每个变量组计算统一图例
        ...
        if unified_value_range is None or (unified_value_range[0] is None and unified_value_range[1] is None):
            ...
            # 检查手动指定的范围有效性
            if len(unified_value_range) != 2 or unified_value_range[0] >= unified_value_range[1]:
    ...
    for year in years:
        for month in months:
            ...
            # -------------------------- 关键修改：参考第一段代码的网格数据处理逻辑 --------------------------
            # 验证数据长度是否匹配模型网格
            if len(df_data) != grid_shape[0] * grid_shape[1]:
                print(f"警告：{year}年{month}月数据长度({len(df_data)})与模型网格尺寸({grid_shape[0]}×{grid_shape[1]}={grid_shape[0]*grid_shape[1]})不匹配")
                print(f"继续处理，但可能存在网格不匹配问题")
            ...
            for variable in available_vars:
                ...
                non_zero_count = (df_data[variable] > 0).sum()
                print(f"  {variable}: 有效数据{valid_count}个，缺失数据{len(df_data)-valid_count}个，大于0的数据{non_zero_count}个")

                # 检查是否所有数据都是0
                if non_zero_count == 0:
                    print(f"  警告：{year}年{month}月 {variable} 变量所有值均为0，跳过绘制")
                    continue
                ...
                # 生成中文标题格式：动态根据变量类型生成标题
                if variable == 'Heatwave_Days':
                    title = f"{month_name_cn} Heatwave Days (≥35°): {year}"
                elif variable == 'Temp_Days_35C':
                    title = f"{month_name_cn} 温度>35°C天数: {year}"
                else:
                ...
                current_unit = plot_settings['unit']
                current_colorbar_format = plot_settings['colorbar_format']

                # 根据变量类型调整单位和格式
                if variable in variable_units:
                    current_unit = variable_units[variable]

                # 根据变量类型设置颜色条格式
                if variable in variable_formats:
                    current_colorbar_format = variable_formats[variable]

                if variable_grouped_legend:
                    # 按变量分组统一图例模式：使用该变量组的统一图例范围
                    ...
                    if var_group in variable_group_ranges:
                        ...
                        print(f"  {year}年{month}月 {variable} 使用{var_group}组统一图例范围：{np.round(current_value_range[0], 2)} ~ {np.round(current_value_range[1], 2)} {current_unit}")
                    else:
                        # 如果组范围不存在，使用该变量的独立范围
                        ...
                        if len(var_data) > 0:
                            ...
                            if len(unique_values) == 1:
                                ...
                                val = unique_values[0]
                                if val == 0:
                                    current_value_range = (0, 1)  # 全为0时的默认范围
                                else:
                                    current_value_range = (val * 0.9, val * 1.1)  # 基于实际值的范围
                            else:
                            ...
                            current_value_range = (0, 1)  # 默认范围
                        print(f"  {year}年{month}月 {variable} 变量组无数据，使用独立图例范围：{np.round(current_value_range[0], 2)} ~ {np.round(current_value_range[1], 2)} {current_unit}")
                elif not unified_legend:
                    ...
                    if len(var_data) > 0:
                        ...
                        if len(unique_values) == 1:
                            ...
                            current_vmax = np.nanpercentile(var_data, 99.5)
                            current_value_range = (current_vmin, current_vmax)
                        print(f"  {year}年{month}月 {variable} 独立图例范围：{np.round(current_value_range[0], 2)} ~ {np.round(current_value_range[1], 2)} {current_unit}")
                    else:
                        current_value_range = (0, 1)  # 默认范围
                        print(f"  {year}年{month}月 {variable} 数据为空，使用默认图例范围")
                ...
                try:
                    fig = show_maps(
                        ...
                        value_range=current_value_range,  # 使用统一或独立的图例范围
...
if __name__ == "__main__":
    ...
    # 3. 变量配置（处理输入数据中的所有列）
    # TARGET_VARIABLES = ['TA_mean', 'SOL_RAD_mean', 'PBLH_mean']
    TARGET_VARIABLES = ['Heatwave_Days','TA_mean_month']

    # 4. 图例配置
    UNIFIED_VALUE_RANGE = (0, 10)  # 图例范围（None则自动计算，或手动指定如(0, 150)）
    UNIFIED_LEGEND = False # 图例模式：True=统一图例，False=分开计算（默认）
    VARIABLE_GROUPED_LEGEND = True  # 按变量分组统一图例（True=按变量组统一，False=使用上面设置）
    ...
    plot_multi_year_maps(
        ...
        key_periods=None,  # 简化格式无需时期筛选
        unified_value_range=UNIFIED_VALUE_RANGE,
        unified_legend=UNIFIED_LEGEND,
        variable_grouped_legend=VARIABLE_GROUPED_LEGEND  # 新增参数
    )
