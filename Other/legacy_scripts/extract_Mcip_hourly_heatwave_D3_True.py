...
FILE_CONFIGS.extend(generate_file_configs(2030, [7]))  # 只测试2023年1月
    ...
    for r in range(1, n_rows + 1):
        for c in range(1, n_cols + 1):
            ...
            grid_data[key] = {
                ...
                'temp_sum': 0.0,           # 目标月份内：小时温度累计和（用于TA_mean_month）
                'temp_count': 0,           # 目标月份内：有效小时数（用于TA_mean_month）
                'sol_rad_values': [],      # 太阳辐射小时数据
                'pblh_values': [],         # 边界层高度小时数据
                'hot_days': 0,             # 高温天数（某天任何小时>35°C）
                'heatwave_days': 0,        # 热浪天数（旧口径：连续>=3天高温时，从第3天起计数；每段长度L贡献L-2）
                'heatwave_days_coverage': 0  # 热浪覆盖天数（推荐口径：连续>=3天高温段内所有天数都计入；每段长度L贡献L）
            }
    return grid_data
...
def extract_meteo_data_from_cmaq(cmaq_file, target_month=7):
    ...
    for local_date_str in sorted(processed_local_dates):
        ...
        # 为TA_mean_month准备：当天24小时温度求和与计数（向量化）
        daily_temps_np = np.asarray(daily_temps)
        daily_temp_sum = np.nansum(daily_temps_np, axis=0)
        daily_temp_count = np.isfinite(daily_temps_np).sum(axis=0)
        ...
        for r in range(n_row):
            for c in range(n_col):
                ...
                meteo_grid_data[key]['temp_sum'] += float(daily_temp_sum[r, c])
                meteo_grid_data[key]['temp_count'] += int(daily_temp_count[r, c])
                if is_hot_day:
...
def create_meteo_csv(grid_data, output_file):
    ...
    for (r, c), data in grid_data.items():
        if data['daily_max_temp']:
            ...
            ta_mean_month = (
                data['temp_sum'] / data['temp_count']
                if data.get('temp_count', 0) not in (0, None)
                else np.nan
            )
            ...
            max_temp_month = ta_mean_month = sol_rad_mean = sol_rad_max = pblh_mean = pblh_max = np.nan
